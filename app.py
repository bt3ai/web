...
def login():
    if request.method == 'POST':
        ...
        if username == USERNAME and password == PASSWORD:
            ...
            return redirect(url_for('schwab'))
        else:
            flash('Invalid username or password', 'error')
    return render_template('login.html')

#what is the issue of mismatch between locl and git?
@app.route('/schwab')
@login_required
def schwab():
    return render_template('index_schwab.html')
...
if __name__ == '__main__':
    ...
    socketio.run(app, host="127.0.0.1", port=9902,  debug=False)
